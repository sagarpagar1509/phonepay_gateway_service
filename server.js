...
const PHONEPE_ORDER_STATUS_URL = 'https://api.phonepe.com/apis/pg/checkout/v2/order';
...
if (!PHONEPE_CLIENT_ID || !PHONEPE_CLIENT_SECRET) {
    console.error('Error: Client ID or Client Secret is missing. Please check your .env file.');
    process.exit(1);
}
...
app.get('/order-status/:orderId', async (req, res) => {
    try {
        ...
        const response = await axios.get(`${PHONEPE_ORDER_STATUS_URL}/${orderId}/status`, {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `O-Bearer ${accessToken}`
            }
